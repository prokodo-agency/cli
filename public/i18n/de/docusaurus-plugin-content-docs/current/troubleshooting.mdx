...
keywords:
  [
    prokodo Fehlerbehebung,
    prokodo CLI Fehler,
    prokodo Fehler beheben,
    prokodo API-Schlüssel Fehler,
    prokodo CLI Debug,
  ]
---
...
| Ursache                                     | Lösung                                         |
| ------------------------------------------- | ---------------------------------------------- |
| Noch nie `auth login` ausgeführt            | `prokodo auth login --key pk_…` ausführen |
...
```
✗ Invalid API key: must start with pk_.
```
...
```bash
# Korrekt
prokodo auth login --key pk_abc123

# Falsch (mit Anführungszeichen als Teil des Werts)
prokodo auth login --key "pk_abc123"  # Anführungszeichen werden von der Shell für Sie entfernt
```
