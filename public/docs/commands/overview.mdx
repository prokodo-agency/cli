...
keywords:
  [
    prokodo CLI commands,
    prokodo auth,
    prokodo init,
    prokodo verify,
    prokodo CLI reference,
    prokodo command list,
  ]
---
...
All commands accept these options before the sub-command:

| Flag              | Type    | Description                                            |
| ----------------- | ------- | ------------------------------------------------------ |
| `--api-key <key>` | string  | Override the API key for this invocation               |
| `--api-url <url>` | string  | Override the API base URL                              |
| `--json`          | boolean | Emit machine-readable JSON on `stdout` instead of text |
| `--no-color`      | boolean | Disable all ANSI colour output                         |
| `--verbose`       | boolean | Stream debug-level info to `stderr`                    |
